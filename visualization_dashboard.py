...
def main():
    ...
    # 9. Riemannian leapfrog integrator result
    print("\n[9] Generating Riemannian leapfrog integrator result...")
    model_lf = GeometryOfAwareness(n=7, seed=42)
    integrator = create_integrator(model_lf, use_christoffel=False)
    x, v = np.random.uniform(-0.3, 0.3, 7), 0.05*np.random.randn(7)
    for _ in range(1000):
        x, v = integrator.step(x, v)
    fig9 = plot_dynamics_trajectory(model_lf, n_steps=1000)
    fig9.savefig('visualizations/09_leapfrog_result.png', dpi=150, bbox_inches='tight')
    print("    [OK] Saved: visualizations/09_leapfrog_result.png")
